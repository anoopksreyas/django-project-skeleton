...
echo "ALREADY STARTED"
  ...
  /usr/local/bin/supervisorctl -c /etc/supervisord/supervisord.conf status
...
stop() {
    ...
    /usr/local/bin/supervisorctl  -c /etc/supervisord/supervisord.conf shutdown
  if ls /tmp/supervisor* &> /dev/null; then
      rm /tmp/supervisor*
  fi
...
case "$1" in
    ...
    restart|force-reload)
      ...
      restart
        ...
        ;;
    reload)
        /usr/local/bin/supervisorctl -c /etc/supervisord/supervisord.conf reload
       RETVAL=$?
        ;;
    condrestart)
